...
export default function Home() {
    ...
    const handleAdd = useCallback((sectionId: string, style: string) => {
        if (sectionId === "header" || sectionId === "footer") {
            // Pour header et footer, remplacer s'ils existent déjà
            setSelectedStyles((prev) => ({ ...prev, [sectionId]: style }))
            setComponentsOrder((prev) => {
                const newOrder = prev.filter((id) => id !== sectionId)
                return sectionId === "header" ? [sectionId, ...newOrder] : [...newOrder, sectionId]
            })
            setEditableContent((prev) => ({ ...prev, [sectionId]: "Edit your content here" }))
        } else {
            // Pour les autres composants, ajouter avec un identifiant unique
            const uniqueId = `${sectionId}-${Date.now()}`
            setSelectedStyles((prev) => ({ ...prev, [uniqueId]: style }))
            setComponentsOrder((prev) => [...prev, uniqueId])
            setEditableContent((prev) => ({ ...prev, [uniqueId]: "Edit your content here" }))
        }
    ...
    const handleMove = useCallback((sectionId: string, direction: "up" | "down") => {
        if (sectionId === "header" || sectionId === "footer") {
            console.log(`Cannot move ${sectionId} component.`)
            return
        }
        setComponentsOrder((prev) => {
            ...
            if (direction === "up" && index > 1) {
                // Empêche de déplacer au-dessus du header
                ;[newOrder[index - 1], newOrder[index]] = [newOrder[index], newOrder[index - 1]]
            } else if (direction === "down" && index < newOrder.length - 2) {
                // Empêche de déplacer en dessous du footer
